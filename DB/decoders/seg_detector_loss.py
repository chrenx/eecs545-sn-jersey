...
class ClassEntropy(nn.Module):
    def __init__(self):
        super(ClassEntropy,self).__init__()
    
    def forward(self,pred: torch.Tensor, gt):
        #weight = torch.tensor([1.5]*100+[0.03], device=torch.device('cuda'))
        loss = nn.CrossEntropyLoss()
        cross = loss(pred['classification'], gt['cls'])
        return cross, dict(cross_entropy_loss=cross)
