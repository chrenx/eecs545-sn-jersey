...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        if len(x.shape) == 5:
            x = rearrange(x, 'b n c h w -> (b n) c h w').contiguous()
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
...
def deformable_resnet50(pretrained=True, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(model_zoo.load_url(
            model_urls['resnet50'], map_location=lambda storage, loc: storage.cuda()), strict=False) # , map_location=lambda storage, loc: storage.cuda()
    return model
