    ...
    def validate(self, validation_loaders, model, epoch, step):
        ...
        for name, loader in validation_loaders.items():
            if self.experiment.validation.visualize:
                ...
                metrics, _ = self.validate_step(loader, model, False)
        ...
        for key, metric in all_matircs.items():
            ...
            #self.logger.info('%s : %f' % (key, metric))
    ...
    def validate_step(self, data_loader, model, visualize=False):
        ...
        for i, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
            with torch.no_grad():
                ...
                mask = pred_classes!=-1
            # self.logger.info(str(targets.shape)+" "+str(pred_classes.shape))
                count += float((targets[mask] == pred_classes[mask]).mean())
                # print(targets, pred)
        
        #all_pred = np.stack(all_pred)
        #all_targets = np.stack(all_targets)
        metric = count / float(len(data_loader))#(all_pred == all_targets).float().mean()
        metrics['acc'] = metric
        return metrics
    '''
    def to_np(self, x):
