        ...
        for j, current_class in enumerate(split_classes[i]):
            ...
            for k in range(config.num_image_per_class):
                ...
                if digits == '00':
                    continue
                if digits[0] == 0:
                    img = image[np.random.choice(indexes[digits[1]])]
                    # imwrite("./test123.png", cv2.resize(np.squeeze(img), (32, 64)))
                    img = cv2.resize(np.squeeze(img), (64, 64))
                else:  # double digits
                    digit_1 = np.squeeze(image[np.random.choice(indexes[digits[0]])])
                    digit_2 = np.squeeze(image[np.random.choice(indexes[digits[1]])])
                    digit_1 = cv2.resize(digit_1, (32,64))
                    digit_2 = cv2.resize(digit_2, (32,64))
                    img = cv2.hconcat([digit_1, digit_2])
                # imwrite("./test123.png", img)
                # exit(0)
...
def argparser():
    ...
    parser.add_argument('--mnist_path', type=str, default='./custom_datasets/mnist/',
                        help='path to *.gz files')
    parser.add_argument('--multimnist_path', type=str, default='./custom_datasets/multimnist')
    ...
    parser.add_argument('--image_size', type=int, nargs='+',
                        default=[64, 64])
    parser.add_argument('--num_image_per_class', type=int, default=1000)
